...
chemh = pd.read_excel('clean_data/chemh_full_tests_clean_cox.xlsx')
print(chemh.head(10))
...
#         if row.Over_7_7 == 1 and id==row.RES_ID and over:
#             chemh.at[index, 'Over_7_7'] = 1
#             print("---------------------------------------INSIDE")
#             over = False
#         elif row.Over_7_7 == 1 and id==row.RES_ID:
...
#this filters the row over_7_7 so that any value after 1, will be deleted
#this is because we want that patient to no longer be in the data, as they now are coded for DM2

chemh = pd.read_excel('clean_data/chemh_full_tests_clean_cox_backup.xlsx')

chemh = chemh[chemh.groupby('RES_ID')['Over_7_7'].apply(lambda x: x.shift().eq(1).cumsum().eq(0))]

# writer = pd.ExcelWriter('clean_Data/chemh_full_tests_clean_cox_test.xlsx', engine='xlsxwriter')
# chemh.to_excel(writer, sheet_name='Sheet1')
# writer.save()