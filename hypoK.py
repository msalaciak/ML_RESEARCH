...
biochem = pd.read_excel('/home/matthew/Research/ML_RESEARCH/hypophosphatemia_clean.xlsx')
progression_info = pd.read_excel('/home/matthew/Research/ML_RESEARCH/clean_data/clean merged/primary-join-follow-up.xlsx')
...
progression_info = progression_info[['ID', 'Progression post RCHOP (yes = 1, no=0)', 'Date Prog after RCHOP for DLBCL', 'DATE_DLBCL Diagnosis','Cause of Death','Date of last Follow-Up']]
...
df=pd.DataFrame()
df['no_relapse']=pd.Series(no_relapse.ID.unique())
df['relapse']=pd.Series(relapsed.ID.unique())
...
filtered_df_no_relapse = prog_update[boolean_series]

boolean_series = prog_update.JGHID.isin(pd.Series(relapsed.ID.unique()))
filtered_df_relapse = prog_update[boolean_series]
