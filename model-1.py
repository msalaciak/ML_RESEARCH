...
nonrelapse = pd.read_excel('clean_Data/nan.xlsx')


print(nonrelapse.head())
print(list(nonrelapse.columns))
print("#########################")

# nonrelapse = nonrelapse.loc[(nonrelapse['#_of_Years_tests_post'] >= 24)]

print(nonrelapse.head(10))

print(relapse.shape)
print(nonrelapse.shape)
...
relapse = relapse.drop(['ID','Unnamed: 0','Date Prog after RCHOP for DLBCL','#_of_Years_tests_post','Test_Date','ORDER'],axis=1)
# nonrelapse = nonrelapse.drop(['ID','Unnamed: 0','DATE_DLBCL Diagnosis','#_of_Years_tests_post','Test_Date','ORDER'],axis=1)
nonrelapse = nonrelapse.drop(['Unnamed: 0'],axis=1)
...
relapse=relapse.reset_index(drop=True)

# print (np.any(np.isnan(relapse)))
# print (np.any(np.isnan(nonrelapse)))

print(nonrelapse.shape)

print(nonrelapse.tail())
...
plt.savefig('ML-MODEL-auc - accuracy.png', dpi=400)
plt.show()
...
plt.savefig('ML-MODEL-feature_importance - accuracy.png', dpi=400)
plt.show()
...
#
# import pydot
# (graph,) = pydot.graph_from_dot_file('/Users/matthew/Desktop/Dr. Johnson Research/ML_RESEARCH/tree.dot')
# graph.write_png('/Users/matthew/Desktop/Dr. Johnson Research/ML_RESEARCH/tree.png')
